    ...
    return (
        <div className={"FeedPostCard "+post.type.replaceAll(' ',"")}>
            <div className="postCardPublish">
                ...
                <a onClick={deleteCard} href="#">Delete</a>
                </div>}
                
            </div>
            {(post.title!==null &&post.title!==undefined &&post.title!=="")? <div className="postCardTitle">{post.title}</div> : null }
            {(post.content!==null &&post.content!==undefined &&post.content!=="")? <p>{post.content}</p> : null }
            {(post.image!==null &&post.image!==undefined &&post.image!=="")? <img alt="" src={post.image}></img> : null }
            
            {(post.link!==null &&post.link!==undefined &&post.link!=="")? <a className='postCardLink' href={post.link}>{post.link}</a> : null }
